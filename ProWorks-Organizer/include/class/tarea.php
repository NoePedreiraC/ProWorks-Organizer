...
function limpiarCampo($campo){
    $mysqli = conecta();
    $limpio = $mysqli->real_escape_string($campo);
    desconecta($mysqli);
    return $limpio;
}
...
// Función para actualizar la fecha de inicio de la tarea en la base de datos
// function actualizarFechaInicioTarea($idTarea, $fechaInicio) {
//     $mysqli = conecta();
    
//     $sql = "UPDATE tarea SET fechaInicio_Tarea = ? WHERE idTarea = '$idTarea'";
    
//     $actuFechIniTare = $mysqli->prepare($sql);
//     if (!$actuFechIniTare) {
//         return false;
//     }

//     $actuFechIniTare->bind_param("si", $fechaInicio, $idTarea);
//     $result = $actuFechIniTare->execute();
//     $actuFechIniTare->close();
//     desconecta($mysqli);
...
// Función para actualizar la fecha de inicio de la tarea
function actualizarFechaInicioTarea($conexion, $idTarea, $fechaInicio) {
    $mysqli = conecta();
    // Verifica si se recibió la fecha de inicio de la tarea
    if (isset($_POST['fechaInicio'])) { 
        // Escapa los datos recibidos para evitar inyección de SQL
        $fechaInicio = limpiarCampo($_POST['fechaInicio']);
        $idTarea = $_POST['idTarea']; // Suponiendo que también envías el ID de la tarea desde JavaScript
        
        // Prepara la consulta SQL para actualizar la fecha de inicio de la tarea
        $sql = "UPDATE tarea SET fechaInicio_Tarea = '$fechaInicio' WHERE idTarea = $idTarea";
...
//___________________________________________________________________
// // Función para actualizar la fecha de fin de la tarea en la base de datos
function actualizarFechaFinTarea( $idTarea, $fechaFin) {
    $mysqli = conecta();
    // Verifica si se recibió la fecha de inicio de la tarea
    if (isset($_POST['fechaFin'])) { 
        // Escapa los datos recibidos para evitar inyección de SQL
        $fechaFin = limpiarCampo($_POST['fechaFin']);
        $idTarea = $_POST['idTarea']; // Suponiendo que también envías el ID de la tarea desde JavaScript
        
        // Prepara la consulta SQL para actualizar la fecha de inicio de la tarea
        $sql = "UPDATE tarea SET fechaFin_Tarea = '$fechaFin' WHERE idTarea = $idTarea";
...
//___________________________________________________________________
function calcularDuracionPromedioTareas($listaTareas) {
    // Inicializar variables para almacenar la duración total y el número de tareas
    $duracionTotal = 0;
    $numTareas = count($listaTareas);

    // Iterar sobre cada tarea en la lista
    foreach ($listaTareas as $tarea) {
        // Obtener la fecha de inicio, fecha de fin y tiempo extra de la tarea
        $fechaInicio = strtotime($tarea['fechaInicio_Tarea']);
        $fechaFin = ($tarea['fechaFin_Tarea']);
        $tiempoExtra = $tarea['tiempoExtra_Tarea'];

        // Calcular la duración de la tarea (fechaFin - fechaInicio + tiempoExtra)
        $duracionTarea = ($fechaFin - $fechaInicio) / (60 * 60); // Convertir a horas
        $duracionTarea += $tiempoExtra;

        // Sumar la duración de la tarea a la duración total
        $duracionTotal += $duracionTarea;
    }

    // Calcular la duración promedio dividiendo la duración total entre el número de tareas
    $duracionPromedio = $duracionTotal / $numTareas;

    // Devolver la duración promedio
...
function obtenerPausaPorId($idPausa){
    $sql = "SELECT * FROM registro_pausa_tarea WHERE idPausa = $idPausa"; 
    $db = conecta();
    if(!$result = $db->query($sql)){
        die('Ha ocurrido un error ejecutando la sentencia obtenerPausaPorIdDeTarea [' . $db->error . ']');
    }
    desconecta($db);
    return mysqli_fetch_object($result);
}

function obtenerPausasPorIdDeTarea($idTareaPausa){
    $sql = "SELECT * FROM registro_pausa_tarea WHERE idTareaPausa = $idTareaPausa"; 
    $db = conecta();
    if(!$result = $db->query($sql)){
...
//___________________________________________________________________________________
 // Función para actualizar la fecha de fin de registro_pausa_tarea en la base de datos
 function actualizarFinPausa($conexion, $idPausa, $finPausa) {
    $mysqli = conecta();
    // Verifica si se recibió la fecha de inicio de la tarea
    if (isset($_POST['finPausa'])) { 
        // Escapa los datos recibidos para evitar inyección de SQL
        $finPausa = limpiarCampo($_POST['finPausa']);
        $idPausa = $_POST['idPausa']; // Suponiendo que también envías el ID de la tarea desde JavaScript
        
        // Prepara la consulta SQL para actualizar la fecha de inicio de la tarea
        $sql = "UPDATE registro_pausa_tarea SET finPausa = '$finPausa' WHERE idPausa = $idPausa";
...
function obtenerIdMaterialPorNombreYTipo($nombreMaterial, $tipoMaterial) {
    // Verificar si el material ya existe en la base de datos
    $sqlBuscarMaterial = "SELECT idMaterial FROM material WHERE nombreMaterial = '$nombreMaterial' AND tipoMaterial = '$tipoMaterial'";
    $resultBuscarMaterial = $mysqli->query($sqlBuscarMaterial);

    if ($resultBuscarMaterial->num_rows > 0) {
        // Si el material existe, obtener su ID
        $row = $resultBuscarMaterial->fetch_assoc();
        $idMaterial = $row['idMaterial'];
    } else {
        $idMaterial = -1; 
    }
    desconecta($mysqli);
    return $resultTareaMaterial;
}
