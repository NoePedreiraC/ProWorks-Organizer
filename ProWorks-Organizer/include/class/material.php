...
function obtenerTipoMateriales(){
	$sql = "SELECT distinct tipoMaterial FROM material"; 
	$db=conecta();
	if(!$result = $db->query($sql)){
		die('Ha ocurrido un error ejecutando la sentencia obtenerMateriales[' . $db->error . ']');
	}
...
function obtenerMaterialPorId($nuevoIdMaterial) {
    $sql = "SELECT * FROM material WHERE idMaterial = '$nuevoIdMaterial'";
    $db = conecta();
    $result = mysqli_query($db, $sql);
    
    if (mysqli_num_rows($result) == 0) {
        $id = agregarMaterial($nuevoIdMaterial);
		obtenerMaterialPorId($nuevoIdMaterial);
    }
	else{
